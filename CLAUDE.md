# SimpleTitle - Minecraft 称号插件

基于 Folia/Paper 的玩家称号系统插件，支持 PlaceholderAPI。

## 项目状态

### 已完成功能

#### 核心功能
- [x] 称号系统（预设 + 自定义）
- [x] 动态称号支持（多内容循环显示）
- [x] PlaceholderAPI 扩展（`%playertitle_current%` 等）
- [x] 数据库支持（H2 本地 / MySQL 跨服）
- [x] 异步数据库队列（非阻塞操作）
- [x] 玩家缓存系统（懒加载 + 触发加载）

#### GUI 系统
- [x] 主菜单（称号列表、分页）
- [x] 称号商店
- [x] 称号详情页（使用、删除、修改边框）
- [x] 边框商城
- [x] 边框选择界面

#### 边框系统
- [x] 预设边框配置（brackets.yml）
- [x] 边框购买（金币/点券）
- [x] 边框权限控制
- [x] 默认边框（所有玩家拥有）
- [x] 修改称号边框

#### 自定义称号
- [x] 静态称号创建
- [x] 动态称号创建（多内容循环）
- [x] 会话管理（多步骤输入、超时）
- [x] 敏感词过滤

#### 数据导入
- [x] CSV 导入命令（`/title import plt <文件名>`）
- [x] PLT 格式支持

#### 数据库事务
- [x] setCurrentTitle 事务支持
- [x] 边框操作回调确认

### 待处理/已知问题

1. **购买操作原子性** 🟡
   - 扣款和给称号不是原子操作
   - 最坏情况：玩家损失金币（概率极低）
   - 暂不处理

## 配置文件

- `config.yml` - 主配置（数据库、自定义称号、动态称号、空格设置）
- `titles.yml` - 预设称号配置
- `brackets.yml` - 预设边框配置
- `messages.yml` - 消息配置

## 命令

| 命令 | 说明 |
|------|------|
| `/title` | 打开称号主菜单 |
| `/title set <ID>` | 设置当前称号 |
| `/title clear` | 清除当前称号 |
| `/title list` | 查看拥有称号 |
| `/title shop` | 打开称号商店 |
| `/title buy <ID>` | 购买预设称号 |
| `/title custom` | 创建自定义称号 |
| `/title brackets` | 打开边框商城 |
| `/title bracket <称号> <边框>` | 修改称号边框 |
| `/title import plt <文件>` | 导入 PLT CSV 数据 |
| `/title reload` | 重载配置 |
| `/title give <玩家> <ID>` | 给予玩家称号 |

## 数据库表结构

### player_titles
- `player_uuid` - 玩家UUID
- `title_id` - 称号ID
- `title_data` - 称号数据（JSON）
- `on_use` - 是否使用中
- `obtained_at` - 获得时间

### player_brackets
- `player_uuid` - 玩家UUID
- `bracket_id` - 边框ID
- `obtained_at` - 获得时间

## 关键文件

```
src/main/java/dev/user/title/
├── SimpleTitlePlugin.java      # 主类
├── command/
│   └── TitleCommand.java       # 命令处理
├── config/
│   └── ConfigManager.java      # 配置管理
├── database/
│   ├── DatabaseManager.java    # 数据库连接池
│   ├── DatabaseQueue.java      # 异步队列
│   └── TitleRepository.java    # 数据访问层
├── economy/
│   ├── EconomyManager.java     # 经济系统
│   └── PlayerPointsManager.java # 点券系统
├── gui/
│   ├── AbstractGUI.java        # GUI 基类
│   ├── TitleMainGUI.java       # 主菜单
│   ├── TitleShopGUI.java       # 商店
│   ├── TitleDetailGUI.java     # 详情页
│   ├── BracketShopGUI.java     # 边框商城
│   └── BracketSelectGUI.java   # 边框选择
├── listener/
│   ├── PlayerListener.java     # 玩家事件
│   └── GUIListener.java        # GUI 事件
├── manager/
│   ├── TitleManager.java       # 称号业务逻辑
│   ├── TitleCacheManager.java  # 称号缓存
│   ├── BracketManager.java     # 边框管理
│   ├── BracketCacheManager.java # 边框缓存
│   ├── DynamicTitleManager.java # 动态称号
│   └── CustomTitleSessionManager.java # 自定义称号会话
├── model/
│   ├── TitleData.java          # 称号数据模型
│   ├── BracketData.java        # 边框数据模型
│   └── TitleType.java          # 称号类型枚举
├── placeholder/
│   └── TitleExpansion.java     # PAPI 扩展
└── util/
    ├── MessageUtil.java        # 消息工具
    └── CsvImporter.java        # CSV 导入工具
```

## 开发注意事项

1. **Folia 兼容**：使用 `GlobalRegionScheduler` 和 `AsyncScheduler`
2. **异步数据库**：所有数据库操作通过 `DatabaseQueue` 异步执行
3. **缓存懒加载**：查询时触发异步加载，避免阻塞主线程
4. **颜色代码**：使用 `&` 格式，通过 `LegacyComponentSerializer.legacyAmpersand()` 解析
